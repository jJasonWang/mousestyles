...
def test_dist_speed_input():
    ...
    assert excinfo.value.args[0] == "Start index must be smaller than end index"
    # Input indices that encompass data outside of true data length
    with pytest.raises(ValueError) as excinfo:
        get_dist_speed(movement, 0, len(movement))
    assert excinfo.value.args[0] == "Number of observations must be less than or equal to total observations"
